...
| `python -m examples.diagnostics codex` | Reports package availability only and exits nonzero if missing. This does not inspect credentials or run a task. |
...
| Command | Expected behavior |
| --- | --- |
| `python -m examples.diagnostics claude --probe-readiness` | Prints package and setup readiness, exiting zero only for `ready-to-attempt`, without running an agent task. |
...
For diagnostics, `not-ready` and `indeterminate` readiness both return a nonzero
status; applications can choose their own policy for `indeterminate`. Use
`--attempt-indeterminate` with the provider task or comparison command to
explicitly attempt a provider whose bounded setup probe could not determine
readiness. A confirmed `not-ready` status still stops. Missing extras, failed
